...
const MyOrders = () => {
  ...
  const handleRating = async (orderId, value) => {
    try {
      const orderDoc = doc(db, "orders", currentUser.uid, "orders", orderId);
      await updateDoc(orderDoc, { rating: value });
      setOrders((prev) =>
        prev.map((o) => (o.id === orderId ? { ...o, rating: value } : o))
      );
    } catch (err) {
          ...
          <div key={order.id} className="card mb-4 shadow-sm border-0">
            ...
            <div className="card-body">
              ...
              {order.status === "delivered" && (
                <div className="mt-3">
                  <h6 className="fw-semibold">Rate Your Order</h6>
                  {order.rating ? (
                    <StarRating rating={order.rating} /> // show readonly
                  ) : (
                    <StarRating
                      rating={0}
                      onRate={(val) => handleRating(order.id, val)}
                    />
                  )}
                </div>
              )}
