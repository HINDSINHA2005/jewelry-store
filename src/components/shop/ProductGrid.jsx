...
// import { Link } from "react-router-dom";
// import { useEffect, useState } from "react";
// import { getAllProducts } from "../shop/getAllProduct"; // merged source

// const ProductGrid = ({ category, searchTerm = "", sortOrder = "" }) => {
//   const [visibleProducts, setVisibleProducts] = useState([]);
//   const [allProducts, setAllProducts] = useState([]);

//   // 🔹 FETCH + NORMALIZE DATA (LOGIC ONLY)
//   useEffect(() => {
//     getAllProducts().then((merged) => {
//       const normalized = merged
//         .filter(p => p.visible !== false)     // ✅ only visible
//         .filter(p => p.stock === undefined || p.stock > 0) // ✅ in stock (firebase-safe)
//         .map((p) => {
//           const cleanPrice = p.price
//             ? Number(String(p.price).replace(/[^\d.]/g, ""))
//             : 0;

//           const cleanSale = p.salePrice
//             ? Number(String(p.salePrice).replace(/[^\d.]/g, ""))
//             : null;
...
//             price: cleanPrice,
//             salePrice: cleanSale,
//             image: p.image || p.imageUrl, // ✅ FIREBASE FIX
//           };
//         });
